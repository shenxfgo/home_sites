...
async def _history(session, user_id, video, progress, completed):
    session.add(
        PlayHistory(
            user_id=user_id, video_id=video.id, progress=progress, completed=completed
        )
    )
    await session.commit()


async def _titles(service, user_id, search=None, **kwargs):
    videos, total = await service.get_videos(user_id, search=search, **kwargs)
    return [video.title for video in videos], total


@pytest.mark.asyncio
async def test_a_keyword_also_matches_the_description_and_tag_names(db_session, user_id):
    ...
    titles, total = await _titles(service, user_id, "悬疑")
...
@pytest.mark.asyncio
async def test_every_keyword_has_to_hit(db_session, user_id):
    source = await _source(db_session, "剧集")
    ...
    assert (await _titles(service, user_id, "第二集 暗涌"))[0] == ["第二集.暗涌"]
    assert (await _titles(service, user_id, "第二集 雨夜"))[0] == ["第二集.暗涌"]
    assert await _titles(service, user_id, "第二集 前夜") == ([], 0)


@pytest.mark.asyncio
async def test_a_quoted_phrase_is_one_keyword(db_session, user_id):
    source = await _source(db_session, "剧集")
    ...
    assert (await _titles(service, user_id, '"dark hero"'))[0] == ["dark hero 收藏版"]
    assert (await _titles(service, user_id, "dark hero"))[0] == ["dark hero 收藏版"]
    assert sorted((await _titles(service, user_id, "hero"))[0]) == sorted(
        ["hero 独行版", "dark hero 收藏版"]
    )


@pytest.mark.asyncio
async def test_source_keyword_limits_to_matching_sources(db_session, user_id):
    ...
    assert (await _titles(service, user_id, "源:剧集"))[0] == ["归途"]
    assert (await _titles(service, user_id, "源:收藏 暗涌"))[0] == ["暗涌"]
    assert await _titles(service, user_id, "源:不存在") == ([], 0)


@pytest.mark.asyncio
async def test_tag_keyword_only_looks_at_tag_names(db_session, user_id):
    source = await _source(db_session, "剧集")
    suspense = await _tag(db_session, "悬疑")
    ...
    assert (await _titles(service, user_id, "标签:悬疑"))[0] == ["暗涌"]


@pytest.mark.asyncio
async def test_rating_and_duration_comparisons(db_session, user_id):
    source = await _source(db_session, "剧集")
    ...
    assert sorted((await _titles(service, user_id, "评分>=4"))[0]) == ["高分短片", "高分长片"]
    assert (await _titles(service, user_id, "评分=0"))[0] == ["新片无分"]
    assert (await _titles(service, user_id, "时长>40分钟"))[0] == ["高分长片"]
    # A unitless number is read as minutes, the unit people quote for films.
    assert (await _titles(service, user_id, "时长>=90"))[0] == ["高分长片"]
    assert (await _titles(service, user_id, "评分>=4 时长<=15分钟"))[0] == ["高分短片"]
...
async def test_watch_state_splits_the_library_three_ways(db_session, user_id):
    ...
    await _history(db_session, user_id, finished, progress=120, completed=True)

    service = VideoService(db_session)

    assert (await _titles(service, user_id, "没看过"))[0] == ["没碰过"]
    assert (await _titles(service, user_id, "未看完"))[0] == ["看一半"]
    assert (await _titles(service, user_id, "已看完"))[0] == ["看完了"]


@pytest.mark.asyncio
async def test_title_hits_rank_above_description_hits(db_session, user_id):
    source = await _source(db_session, "剧集")
    ...
    service = VideoService(db_session)
    titles, _ = await _titles(service, user_id, "暗涌")
...
@pytest.mark.asyncio
async def test_wildcards_typed_by_the_user_are_literal(db_session, user_id):
    source = await _source(db_session, "剧集")
    ...
    assert (await _titles(service, user_id, "100%"))[0] == ["100% 胶片"]
    assert (await _titles(service, user_id, "A_B"))[0] == ["A_B 修复版"]


@pytest.mark.asyncio
async def test_input_that_is_not_an_operator_stays_a_keyword(db_session, user_id):
    source = await _source(db_session, "剧集")
    await _video(db_session, source, "16:9 修复版")

    service = VideoService(db_session)

    assert (await _titles(service, user_id, "16:9"))[0] == ["16:9 修复版"]


@pytest.mark.asyncio
async def test_a_video_counted_by_two_tags_is_returned_once(db_session, user_id):
    source = await _source(db_session, "剧集")
    suspense = await _tag(db_session, "悬疑")
...
@pytest.mark.asyncio
async def test_search_combines_with_the_dropdown_filters(db_session, user_id):
